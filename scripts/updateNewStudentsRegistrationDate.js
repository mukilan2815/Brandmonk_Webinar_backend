...
function normalizeName(name) {
  return name.trim().toUpperCase().replace(/\s+/g, ' ').replace(/\./g, '').replace(/,/g, '');
}
...
async function main() {
  try {
    ...
    const targetDate = new Date('2026-07-18T00:00:00.000+05:30');
    let updated = 0;

    for (const s of newStudents) {
      const normalizedName = normalizeName(s.name);
      const result = await CourseStudent.updateOne(
        {
        ...
        { $set: { dateOfRegistration: targetDate } }
      );

      if (result.matchedCount > 0) {
        console.log(`✅ Updated ${s.name} (${s.courseSlug}) dateOfRegistration → 18-07-2026`);
        updated += result.modifiedCount;
      } else {
        console.log(`❌ Not found: ${s.name} (${s.courseSlug})`);
      }
    }

    console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`✅ Updated ${updated} students' registration date`);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  } catch (error) {
